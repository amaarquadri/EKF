...
Wrench<double> getAppliedLoads(const bmb_msgs::AircraftState& state,
  ...
  const Wrench<double> aileron_loads =
      wrenchFromAileron(body_vel, control_inputs.right_aileron_angle);
  const Wrench<double> elevator_loads =
      wrenchFromElevator(body_vel, control_inputs.elevator_angle);
  const Wrench<double> rudder_loads = wrenchFromRudder(body_vel, sin_aoa_xy);
